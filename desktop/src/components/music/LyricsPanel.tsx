...
import { lrclibGet, lrclibSearch } from '../../lib/lrclib';
import {
  type LyricLine,
  type LyricsSource,
  searchLyricsManual,
...
const SyncedLyrics = React.memo(({ lines }: { lines: LyricLine[] }) => {
  ...
  useEffect(() => {
    ...
    let rafId = 0;
    let lastTickTs = 0;
    const FRAME_BUDGET_MS = 7; // ~144fps target
...
const TimedCommentsRail = React.memo(({ trackUrn }: { trackUrn: string }) => {
  ...
  useEffect(() => {
    ...
    // Only tick while playing — pause the interval when audio is paused to save CPU.
    ...
    else syncActiveIndex(); // sync once so position is correct even while paused
...
const LyricsPane = React.memo(({ track }: { track: Track }) => {
  ...
  const { data: lyrics, isLoading } = useQuery({
    queryKey: manualQuery
      ...
      : lyricsQueryKey(track),
    queryFn: async () => {
      // Manual search: user provided custom artist/title
      if (manualQuery) {
      ...
      // Normal track: use the shared fetch (disk cache → network)
  ...
  if (lyrics?.synced && lyrics.synced.length > 0) {
    return (
      <>
        <LyricsSourceBadge source={lyrics.source} hasSynced={true} onSearch={startSearch} />
  ...
  if (lyrics?.plain) {
    return (
      <>
        <LyricsSourceBadge source={lyrics.source} hasSynced={false} onSearch={startSearch} />
