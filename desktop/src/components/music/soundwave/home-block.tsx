...
export const SoundWaveBlock = React.memo(function SoundWaveBlock() {
  ...
  const { data, isLoading, isFetching, isError, refetch } = useSoundWave({
  ...
  const showCold = !isSearchMode && !isLoading && !isError && recTracks.length === 0;
  const showError = !isSearchMode && isError && recTracks.length === 0;
          ...
          {isSearchMode ? (
            ...
            <>
              ...
              {isLoading ? (
                ...
                <EmptyState
                  icon={<RefreshCw size={16} style={{ color: 'var(--color-accent)' }} />}
                  title={t('soundwave.errorTitle', 'Connection error')}
                  desc={t('soundwave.errorDesc', 'Could not load recommendations. Click refresh to retry.')}
                />
              ) : showCold ? (
                <EmptyState
