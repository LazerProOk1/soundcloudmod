...
export const ProgressSlider = React.memo(() => {
  // useEffect+useState instead of useSyncExternalStore — avoids React 19 "forceStoreRerender"
  // loop when audio:tick fires at 60 fps between render and commit phases.
  const [duration, setDuration] = useState(getDuration);
  useEffect(() => subscribe(() => setDuration(getDuration())), []);
...
const SLEEP_PRESETS_MIN = [5, 10, 15, 30, 45, 60] as const;
...
const SleepTimerBtn = React.memo(() => {
  const { t } = useTranslation();
  const { endsAt, afterTrack, set, setAfterTrack, cancel } = useSleepTimer();
  const isActive = endsAt !== null || afterTrack;

  // Countdown that ticks every second while a fixed timer is running
  ...
  useEffect(() => {
    ...
    setRemaining(getSleepTimerRemaining());
    ...
    <Popover.Root>
      <Popover.Trigger asChild>
        <button
          ...
          className={`relative ${btnClass(isActive, 'sm')}`}
        >
          <Moon size={16} />
          {/* small dot indicator when active */}
          {isActive && (
            <span className="absolute top-1.5 right-1.5 h-1.5 w-1.5 rounded-full bg-accent shadow-[0_0_6px_var(--color-accent-glow)]" />
          )}
        </button>
      </Popover.Trigger>
      <Popover.Portal>
            ...
            <div className="min-w-0">
              ...
              {isActive && (
                <p className="text-[10px] text-accent">
                  {afterTrack ? t('player.sleepAfterTrack') : fmtCountdown(remaining ?? 0)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="relative space-y-1">
            ...
            {isActive && (
              <button
                type="button"
                onClick={cancel}
                className="w-full rounded-[10px] bg-rose-500/[0.12] py-2 text-[12px] font-semibold text-rose-400 transition-colors hover:bg-rose-500/[0.20] cursor-pointer"
              >
              ...
              {SLEEP_PRESETS_MIN.map((min) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => set(min)}
                    className={`rounded-[10px] py-2 text-[12px] font-semibold transition-all cursor-pointer ${
                      isSelected
            ...
            <button
              type="button"
              onClick={setAfterTrack}
              className={`w-full rounded-[10px] py-2 text-[12px] font-semibold transition-all cursor-pointer ${
                afterTrack
                  ? 'bg-accent/[0.18] text-accent'
                  : 'bg-white/[0.04] text-white/60 hover:bg-white/[0.08] hover:text-white/90'
              }`}
            >
              {t('player.sleepAfterTrack')}
            </button>
          </div>
        </Popover.Content>
      </Popover.Portal>
